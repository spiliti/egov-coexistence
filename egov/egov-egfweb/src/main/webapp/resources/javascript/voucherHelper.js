...
function openSearchWindowFromJV(obj) {
	...
	if( detailtypeid != null && detailtypeid != 0) {
		...
		bootbox.alert(jQuery.i18n.prop("select.type.message"));
	}
}
...
function popupCallback(arg0, srchType) {
	...
	if(srchType == 'EntitySearch' ) {
		...
		{
			bootbox.alert(jQuery.i18n.prop("invalid.entity.selected.message"));
...
var callbackCommon = {
	success: function(o) {
		...
		{
			bootbox.alert(jQuery.i18n.prop('enter.valid.code.message'));
			document.getElementById('detailKey').value='';
			document.getElementById('detailName').value='';
			return;
		}
    ...
    failure: function(o) {
    	bootbox.alert(jQuery.i18n.prop('failure.message'));
    }
}
...
function validate()
{
	...
	{
		bootbox.alert(jQuery.i18n.prop('total.debit.amount.cant.be.zero.message'));
		return false;
	}
	if(crtotal=='' || crtotal==0)
	{
		bootbox.alert(jQuery.i18n.prop('total.credit.amount.cant.be.zero.message'));
		return false;
	}
	if(dbtotal!=crtotal)
	{
		bootbox.alert(jQuery.i18n.prop('total.debit.credit.amount.should.not.same.message'));
		return false;
	}
	return true;
}
...
function checkLength(obj)
{
	...
	{
		bootbox.alert(jQuery.i18n.prop('max.charatcers.warning.for.narration.message'))
...
function checkVoucherNarrationLen(obj)
{
	...
	{
		bootbox.alert(jQuery.i18n.prop('max.charatcers.warning.for.comment.message'))
...
function validateDigits(obj)
{
	...
	{
		bootbox.alert(jQuery.i18n.prop("invalid.amount.message"))
		obj.value=0;
		obj.focus();
	}
	else
	{
		...
		{
			bootbox.alert(jQuery.i18n.prop("negative.amount.not.allowed.message"));
			obj.value=0;
			obj.focus();
		}
		else
...
var callbackProjectCodes = {
    ...
    failure: function(o) {
    	bootbox.alert(jQuery.i18n.prop('failed.to.load.project.codes.message'));
    }
}
